...
function formatBytes(a, b) { if (a == 0) return '0 Bytes'; const c = 1024; const d = b || 2; const e = ['Bytes', 'KB', 'MB', 'GB', 'TB', 'PB', 'EB', 'ZB', 'YB']; const f = Math.floor(Math.log(a) / Math.log(c)); return `${parseFloat((a / Math.pow(c, f)).toFixed(d))} ${e[f]}`; }
  ...
  return new Promise((resolve, reject) => {
    ...
    factorioAPI.authenticate({
      ...
      factorioAPI.searchMods(searchTerm).then((mods) => {
        const result = asTable(mods.reverse().map((mod) => {
          return {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (term && term.length > 0) {
      if (term.includes('mods.factorio.com/mods/')) { // https://mods.factorio.com/mods/Danielv123/ChooChoo
        ...
        const apiURL = `https://api.github.com/repos/${term.split('/')[3]}/${term.split('/')[4]}/releases`; // https://api.github.com/repos/Danielv123/factorioClusterioMod/releases

        const res = syncRequest('GET', apiURL, { headers : { 'User-Agent' : 'Fuck you for requiring user agents!' } });
        const url = JSON.parse(res.getBody())[0].assets[0].browser_download_url;
        const name = JSON.parse(res.getBody())[0].assets[0].name;
        if (url) {
          console.log(`Downloading ${url}`);
          const file = fs.createWriteStream(modDirectory + name);
          let bytesDownloaded = 0;
          const downloadStartTime = Date.now();
          https.get(url, (response) => {
            const bytesToDownload = response.headers['content-length'];
            response.pipe(file);
            response.on('data', (chunk) => {
              bytesDownloaded += chunk.length;
              const downloadTimeElapsed = Date.now() - downloadStartTime;
              const bytesPerSecond = bytesDownloaded / (downloadTimeElapsed / 1000);
              const timeToComplete = Math.floor((bytesToDownload - bytesDownloaded) / bytesPerSecond);
...
module.exports.removeMod = function removeMod(term, instance, force = false) {
  return new Promise((resolve, reject) => {
    ...
    if (term && typeof term === 'string' && term.length > 0) {
      const mods = fs.readdirSync(modDirectory);
      const modsMatchingFilter = mods.filter(mod => mod.toLowerCase().includes(term.toLowerCase()));
      if (modsMatchingFilter.length < 1) {
        ...
        if (query('Delete all mods matching filter?')) {
          const modsToDelete = modsMatchingFilter.length;
...
module.exports.downloadFromGitURL = function (url) {
  ...
  const res = syncRequest('GET', url, { headers : { 'User-Agent' : "Please don't require setting a user agent" } });
  const downloadUrl = JSON.parse(res.getBody())[0].assets[0].browser_download_url;
  const name = JSON.parse(res.getBody())[0].assets[0].name;
  console.log(JSON.parse(res.getBody()));
  if (downloadUrl) {

...
app.get('/metrics', (req, res) => {
  ...
  for (const instanceID in slaves) {
    // playercount
    try {
...
db.signals.ensureIndex({ fieldName : 'time', expireAfterSeconds : 3600 }, (err) => {});
...
(function () {
  try {
    const x = fs.statSync(path.resolve(config.databaseDirectory, '/slaves.json'));
...
process.on('SIGINT', async () => {
  ...
  for (const i in masterPlugins) {
    const plugin = masterPlugins[i];
...
@alias /api/getID
*/
app.post('/api/getID', authenticate.middleware, (req, res) => {
  endpointHitCounter.labels(req.route.path).inc();
  const reqStartTime = Date.now();
...
*/
app.post('/api/editSlaveMeta', authenticate.middleware, (req, res) => {
  endpointHitCounter.labels(req.route.path).inc();
  const reqStartTime = Date.now();
  // request.body should be an object
...
*/
app.post('/api/getSlaveMeta', (req, res) => {
  endpointHitCounter.labels(req.route.path).inc();
  const reqStartTime = Date.now();
...
@alias /api/checkMod
*/
app.post('/api/checkMod', (req, res) => {
  endpointHitCounter.labels(req.route.path).inc();
  const reqStartTime = Date.now();
  const files = fs.readdirSync(masterModFolder);
  let found = false;
  files.forEach((file) => {
...
@alias /api/uploadMod
*/
app.post('/api/uploadMod', authenticate.middleware, (req, res) => {
  endpointHitCounter.labels(req.route.path).inc();
  const reqStartTime = Date.now();
  if (req.files && req.files.file) {
    console.log(req.files.file);
    req.files.file.mv(path.resolve(config.databaseDirectory, 'masterMods', req.files.file.name), (err) => {
      if (err) {
...
app.get('/api/slaves', (req, res) => {
  ...
  if (!slaveCache.cache || Date.now() - slaveCache.timestamp > 5000) {
    const copyOfSlaves = JSON.parse(JSON.stringify(slaves));
...
const recievedItemStatisticsBySlaveID = {};
const sentItemStatisticsBySlaveID = {};
/*
...
*/
app.post('/api/place', authenticate.middleware, (req, res) => {
  endpointHitCounter.labels(req.route.path).inc();
  const reqStartTime = Date.now();
  res.header('Access-Control-Allow-Origin', '*');
  res.header('Access-Control-Allow-Headers', 'Origin, X-Requested-With, Content-Type, Accept');
...
app.post('/api/remove', authenticate.middleware, (req, res) => {
  endpointHitCounter.labels(req.route.path).inc();
  const reqStartTime = Date.now();
  ...
  const item = db.items[object.name];
  // console.dir(doc);
  if (!item) {
    if (config.logItemTransfers) {
...
*/
app.post('/api/setSignal', authenticate.middleware, (req, res) => {
  endpointHitCounter.labels(req.route.path).inc();
  const reqStartTime = Date.now();
...
*/
app.post('/api/readSignal', (req, res) => {
  endpointHitCounter.labels(req.route.path).inc();
  const reqStartTime = Date.now();
  // request.body should be an object
  // {since:UNIXTIMESTAMP,}
  // we should send an array of all signals since then
  db.signals.find({ time : { $gte : req.body.since } }, (err, docs) => {
...
*/
app.post('/api/logStats', authenticate.middleware, (req, res) => {
  endpointHitCounter.labels(req.route.path).inc();
  const reqStartTime = Date.now();
...
app.post('/api/getStats', (req, res) => {
  ...
  if (typeof req.body === 'object' && req.body.instanceID && req.body.statistic === undefined) {
    ...
    if (req.body.statistic == 'place') {
      ...
      const itemStats = recievedItemStatisticsBySlaveID[req.body.instanceID];
      if (itemStats === undefined) {
        res.send({ statusForDebugging : 'no data available' });
        return false;
      }
      const data = itemStats.get(config.itemStats.maxEntries, req.body.itemName);
      ...
      const itemStats = sentItemStatisticsBySlaveID[req.body.instanceID];
...
*/
app.post('/api/getTimelineStats', (req, res) => {
  endpointHitCounter.labels(req.route.path).inc();
  const reqStartTime = Date.now();
  console.log(req.body);
...
app.post('/api/runCommand', (req, res) => {
  ...
  jwt.verify(token, config.masterAuthSecret, (err, decoded) => {
    ...
    if (
      ...
      for (const instanceID in wsSlaves) {
...
*/
app.get('/api/getFactorioLocale', (req, res) => {
  endpointHitCounter.labels(req.route.path).inc();
  const reqStartTime = Date.now();
...
if (args.sslPort || config.sslPort) {
  try {
    const certificate = fs.readFileSync(path.resolve(config.databaseDirectory, 'certificates/cert.crt'));
    const privateKey = fs.readFileSync(path.resolve(config.databaseDirectory, 'certificates/cert.key'));
...
if (args.masterPort || config.masterPort) {
  ...
  server.listen(args.masterPort || config.masterPort || 8080, () => {
  ...
  constructor(instanceID, socket) {
    ...
    this.socket.on('sendEntity', (entity) => {
      ...
      Object.keys(mapRequesters).forEach((requesterName) => {
        const requester = mapRequesters[requesterName];
...
class mapRequester {
  constructor(requesterID, socket, instanceID) {
    ...
    this.socket.on('requestChunk', (loc) => {
      ...
      const instanceID = loc.instanceID || this.instanceID;
    ...
    this.socket.on('requestEntity', (req) => {
      ...
      const instanceID = req.instanceID || this.instanceID;
...
io.on('connection', (socket) => {
  ...
  const currentConnections = Object.keys(mapRequesters).length + Object.keys(slaveMappers).length + Object.keys(wsSlaves).length;
  [mapRequesters, slaveMappers, wsSlaves].forEach((list) => {
    Object.keys(list).forEach((connectionID) => {
      const connection = list[connectionID];
  ...
  socket.on('registerSlaveMapper', (data) => {
  ...
  socket.on('registerMapRequester', (data) => {
    // data {instanceID:""}
    prometheusWsUsageCounter.labels('registerMapRequester', 'other').inc();
    const requesterID = Math.random().toString();
  ...
  socket.on('registerAlertReciever', (data) => {
...
async function pluginManagement() {
  const startPluginLoad = Date.now();
...
function getPlugins(pluginDirectory) {
  return new Promise((resolve, reject) => {
    ...
    fs.readdir(pluginDirectory, (err, files) => {
      if (err) {
        reject(err);
      } else {
        files.forEach((file) => {
          const pluginStartedLoading = Date.now(); // just for logging
          filesLeft++;
          ...
          fs.stat(pluginPath, (err, stats) => {
            if (err) {
              ...
              const pluginConfig = require(path.resolve(pluginPath, 'config'));
              --filesLeft;
              if (pluginConfig.masterPlugin) {
                const masterPlugin = require(path.resolve(pluginPath, pluginConfig.masterPlugin));
                plugins.push({

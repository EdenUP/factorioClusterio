...
async function messageInterfaceInternal(command, callback, resolve, reject) {
  ...
  if (false && process.platform == 'linux' && typeof command === 'string' && serverprocess) {
    ...
    try {
      const str = await client.send(`${command}\n`);
...
if (!command || command == 'help' || command == '--help') {
  ...
  const instanceNames = fileOps.getDirectoriesSync(config.instanceDirectory);
  ...
  instanceNames.forEach((instance, index) => {
  ...
  const res = syncRequest('GET', 'https://api.github.com/repos/Danielv123/factorioClusterioMod/releases', { headers : { 'User-Agent' : 'Fuck you for requiring user agents!' } });
  const url = JSON.parse(res.getBody())[0].assets[0].browser_download_url;
  const name = JSON.parse(res.getBody())[0].assets[0].name;
  if (url) {
    console.log(url);
    const file = fs.createWriteStream(`sharedMods/${name}`);
...
write-data=${path.resolve(config.instanceDirectory, instance)}\r\n
  ...
  const createSave = child_process.spawnSync(
  ...
  const savefiles = fs.readdirSync(`${instancedirectory}/saves/`);
  ...
  fileOps.getNewestFile(`${instancedirectory}/saves/`, fs.readdirSync(`${instancedirectory}/saves/`), (err, latestSave) => {
    if (err) {
      console.error('ERROR!');
 ...
 installed and configured properly. Try installing factorio and adding your savefile to instances/[instancename]/saves/\n');
    ...
    if (true || process.platform != 'linux') {
      ...
      function checkRcon() {
        fs.readFile(`${instancedirectory}/factorio-current.log`, (err, data) => {
...
function instanceManagement() {
  ...
  const socket = ioClient(`http://${config.masterIP}:${config.masterPort}`);
  ...
  const pluginDirectories = fileOps.getDirectoriesSync('./sharedPlugins/');
  const plugins = [];
  for (let i = 0; i < pluginDirectories.length; i++) {
    ...
    const combinedConfig = deepmerge(instanceconfig, config, { clone : true });
    const pluginConfig = require(`./sharedPlugins/${pluginDirectories[i]}/config.js`);
    ...
    if (pluginConfig.binary == 'nodePackage') {
      ...
      const pluginClass = require(`./sharedPlugins/${pluginDirectories[I]}/index.js`);
      plugins[I] = new pluginClass(combinedConfig, (async (data, callback) => {
          ...
          if (filename != null) {
            setTimeout(
              () => {
                ...
                const stuff = fs.readFileSync(`${instancedirectory}/script-output/${filename}`, 'utf8').split('\n');
      ...
      if (pluginConfig.scriptOutputFileSubscription && typeof pluginConfig.scriptOutputFileSubscription === 'string') {
        ...
        global.subscribedFiles[pluginConfig.scriptOutputFileSubscription] = true;
        fs.watch(`${instancedirectory}/script-output/${pluginConfig.scriptOutputFileSubscription}`, (eventType, filename) => {
          // get array of lines in file
          // TODO async
          const stuff = fs.readFileSync(`${instancedirectory}/script-output/${filename}`, 'utf8').split('\n');
          // if you found anything, reset the file
  ...
  // world IDs ------------------------------------------------------------------
  hashMods(instance, (modHashes) => {
    setInterval(getID, 10000);
    getID();
    function getID() {
      messageInterface('/silent-command rcon.print(#game.connected_players)', (playerCount) => {
        const payload = {
          time         : Date.now(),
        ...
        function callback(err, mac) {
          ...
          needle.post(`${config.masterIP}:${config.masterPort}/api/getID`, payload, needleOptionsWithTokenAuthHeader, (err, response, body) => {
  ...
  setTimeout(() => { hashMods(instance, uploadMods); }, 5000);
  function uploadMods(modHashes) {
    ...
    for (let i = 0; i < modHashes.length; i++) {
      ...
      needle.post(`${config.masterIP}:${config.masterPort}/api/checkMod`, payload, needleOptionsWithTokenAuthHeader, (err, response, body) => {
        ...
        if (response && body && body == 'found') {
          ...
          if (config.uploadModsToMaster) {
            ...
            const req = request.post({
              url     : `http://${config.masterIP}:${config.masterPort}/api/uploadMod`,
              headers : {
                'x-access-token' : config.masterAuthToken,
              },
            }, (err, resp, body) => {
  ...
  oldFlowStats = false;
  setInterval(() => {
    fs.readFile(`${instancedirectory}/script-output/flows.txt`, { encoding : 'utf8' }, (err, data) => {
      if (!err && data) {
        ...
        // fluids
        const flowStat1 = flowStats[flowStats.length - 1].flows.player.input_counts;
        // items
        const flowStat2 = flowStats[flowStats.length - 2].flows.player.input_counts;
        // merge fluid and item flows
        const totalFlows = {};
        for (const key in flowStat1) totalFlows[key] = flowStat1[key];
        for (const key in flowStat2) totalFlows[key] = flowStat2[key];
        if (oldFlowStats && totalFlows && oldTimestamp) {
          const payload = objectOps.deepclone(totalFlows);
          // change from total reported to per time unit
          for (const key in oldFlowStats) {
          ...
          needle.post(`${config.masterIP}:${config.masterPort}/api/logStats`, { timestamp, instanceID : instanceconfig.unique, data : payload }, needleOptionsWithTokenAuthHeader, (err, response, body) => {
  ...
  fs.watch(`${instancedirectory}/script-output/output.txt`, (eventType, filename) => {
    // get array of lines in file
    const items = fs.readFileSync(`${instancedirectory}/script-output/output.txt`, 'utf8').split('\n');
    // if you found anything, reset the file
    if (items[0]) {
      fs.writeFileSync(`${instancedirectory}/script-output/output.txt`, '');
    }
    for (let i = 0; i < items.length; i++) {
      if (items[i]) {
        ...
        }, needleOptionsWithTokenAuthHeader, (err, resp, body) => {
  ...
  setInterval(() => {
    // get array of lines in file
    const items = fs.readFileSync(`${instancedirectory}/script-output/orders.txt`, 'utf8').split('\n');
    ...
    if (items[0]) {
      ...
      for (let i = 0; i < Object.keys(preparedPackage).length; i++) {
        ...
        needle.post(`${config.masterIP}:${config.masterPort}/api/remove`, preparedPackage[Object.keys(preparedPackage)[i]], needleOptionsWithTokenAuthHeader, (err, response, body) => {
  ...
  setInterval(() => {
    needle.get(`${config.masterIP}:${config.masterPort}/api/inventory`, (err, response, body) => {
      if (err) {
        ...
        try {
          ...
          let first = true;
          let cmd = 'local s={';
          for (const key in inventoryFrame) {
  ...
  const signals = fs.readFileSync(`${instancedirectory}/script-output/txbuffer.txt`, 'utf8').split('\n');
  // if we actually got anything from the file, proceed and reset file
          ...
          if (signals[0]) {
            ...
            for (let i = 0; i < signals.length; i++) {
              if (signals[i] && objectOps.isJSON(signals[i])) {
                ...
                const framepart = JSON.parse(signals[i]);
                const doneframe = {
                  time  : Date.now(),
...
function hashMods(instanceName, callback) {
  ...
  for (let i = 0; i < instanceMods.length; i++) {
    const path = `${config.instanceDirectory}/${instanceName}/mods/${instanceMods[i]}`;
    const name = instanceMods[i];
    const options = {
      files : path,
    };

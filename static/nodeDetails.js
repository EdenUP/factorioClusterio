...
function sortByKey(array, key) {
  return array.sort((a, b) => {
    const x = Number(a[key]); const y = Number(b[key]);
...
setInterval(() => {
  getJSON('/api/slaves', (err, slaveData) => {
    const indicator = document.querySelector('#onlineIndicator');
    if (err) {
...
function populateSlaveInfo() {
  ...
  getJSON('/api/slaves', (err, slaveData) => {
    /*
		...
		*/
    const slave = slaveData[instanceID];
    ...
    HTML = '';
    if (slave.meta) {
      for (const key in slave.meta) {
        const t = slave.meta[key];
        if (typeof t === 'string') {
          HTML += `<p>${key}: ${t}</p>`;
        }
      }
...
function makeTerminal() {
  ...
  function handleTerminalInput(inputString) {
    ...
    const lastLine = document.querySelector('#terminal div > p:nth-child(1) > div:last-child');
...
function maximizeTerminal(instant) {
  ...
  const oldTerminalPosition = JSON.parse(localStorage.oldTerminalPosition);
...
function makeGraph(instanceID, selector) {
  const chartIgnoreList = [
    'water',
    'steam',
  ];
  post('api/getStats', { instanceID }, (data) => {
    // console.log("Building chart " + instanceID + " with this data:")
    // console.log(data)
    if (data.length > 0) {
      // find keys
      const itemNames = [];
      for (const key in data[data.length - 1].data) {

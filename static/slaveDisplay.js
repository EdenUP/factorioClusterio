...
setTimeout(() => {
  ...
  xmlhttp.onreadystatechange = function () {
    if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
      const slaveData = JSON.parse(xmlhttp.responseText);
      let HTML = '<p>Current connections:</p>';
      for (i = 0; i < Object.keys(slaveData).length; i++) {
        const key = Object.keys(slaveData)[i];
        ...
        if (Date.now() - slaveData[key].time < 120000 || displayOffline) {
          const seenDate = moment(Number(slaveData[key].time)).format('DD.MM.YY, hh:mm:ss');
      ...
      if (JSON.parse(localStorage.settings)['Enable production graphs']) {
        const graphs = document.querySelectorAll('.productionGraph');
...
function makeGraph(instanceID, selector) {
  post('api/getTimelineStats', { instanceID }, (pointsInTime) => {
    ...
    const displayNames = itemNames.filter((name) => {
      return !chartIgnoreList.includes(name);
    });
    const chartData = displayNames.map((name) => {

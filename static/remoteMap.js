...
const itemSelectorGUI = new itemSelector('#remoteMapItemSelector', [
  { name : 'stone-furnace' },
  { name : 'accumulator' },
...
const remoteMapConfig = JSON.parse(localStorage.remoteMapConfig);

// Handle config changes through UI buttons
(function () {
  const selectMapSize = document.querySelector('#mapSize');
  const selectTileSize = document.querySelector('#tileSize');
  if (selectMapSize && selectTileSize) {
    ...
    selectMapSize.onchange = function () {
      const config = JSON.parse(localStorage.remoteMapConfig);
...
socket.on('hello', (data) => {
  ...
  socket.on('displayChunk', (chunk) => {
    console.log("displayChunk triggered but I can't draw chunks");
  });
  socket.on('displayEntity', (entity) => {
...
function requestChunk(x, y) {
  socket.emit('requestChunk', { x, y, instanceID : getParameterByName('instanceID') });
}
(function () {
  const remoteMapLayers = document.querySelectorAll('.remoteMap');
...
function requestMapDraw() {
  const xLow = Math.floor(cachePosition.x % remoteMapConfig.tileSize);
  const yLow = Math.floor(cachePosition.y % remoteMapConfig.tileSize);

  const xHigh = xLow + remoteMapConfig.mapSize;
  const yHigh = yLow + remoteMapConfig.mapSize;
...
const entityCache = new Array(remoteMapConfig.mapSize);
...
const cache = {
  walkDown : function walkUp() {
    ...
    for (let i = 0; i < remoteMapConfig.mapSize; i++) {
      ...
      const x = cachePosition.x / remoteMapConfig.tileSize + i;
      const y = cachePosition.y / remoteMapConfig.tileSize + remoteMapConfig.mapSize - 1;
      socket.emit('requestEntity', { x, y, instanceID : getParameterByName('instanceID') });
    }
  },
  walkUp : function walkDown() {
    ...
    for (let i = 0; i < remoteMapConfig.mapSize; i++) {
      ...
      // get data from slaveMapper
      const x = cachePosition.x / remoteMapConfig.tileSize + i;
      const y = cachePosition.y / remoteMapConfig.tileSize;
      socket.emit('requestEntity', { x, y, instanceID : getParameterByName('instanceID') });
    }
  },
  walkLeft : function walkLeft() {
    ...
    for (let i = 0; i < remoteMapConfig.mapSize; i++) {
      const x = cachePosition.x / remoteMapConfig.tileSize + remoteMapConfig.mapSize - 1;
      const y = cachePosition.y / remoteMapConfig.tileSize + i;
      socket.emit('requestEntity', { x, y, instanceID : getParameterByName('instanceID') });
    }
  },
  walkRight : function walkRight() {
    ...
    for (let i = 0; i < remoteMapConfig.mapSize; i++) {
      const x = cachePosition.x / remoteMapConfig.tileSize;
      const y = cachePosition.y / remoteMapConfig.tileSize + i;
      socket.emit('requestEntity', { x, y, instanceID : getParameterByName('instanceID') });
    }
  },
...
window.mousePos = {};
selectionCanvas.addEventListener('mousemove', (evt) => {
  const mousePos = getMousePos(selectionCanvas, evt);
...
selectionCanvas.addEventListener('mousedown', (evt) => {
  ...
  const tileSize = remoteMapConfig.tileSize;
  const entity = {
...
selectionCanvas.addEventListener('mouseup', (evt) => {
...
function renderLoop() {
  ...
  if (!isPaused) {
    const moveSpeed = Number(remoteMapConfig.movementSpeed);
    const movement = moveSpeed * timeSinceLastFrame;
  ...
  const mousePosition = window.mousePos;
  if (mousePosition && mousePosition.x) {
    ...
    const tilePosition = { // position in world coordinates (same as used by factorio)
  ...
  const dw = 1024 / 32 * remoteMapConfig.tileSize;
  const dh = 64 / 32 * remoteMapConfig.tileSize;
  ...
  for (let x = 0 - dw; x < remoteMapConfig.tileSize * remoteMapConfig.mapSize + dw; x += dw) {
    for (let y = 0 - dh; y < remoteMapConfig.tileSize * remoteMapConfig.mapSize + dh; y += dh) {
      const dx = x - playerPosition.x % dw;
      const dy = y - playerPosition.y % dh;
...
const entityImages = {}; // cache to store images and details about entities, populated by drawEntity();
    ...
    if (entity.entity && entity.entity.name && typeof entity.entity.name === 'string') {
      ...
      if (!entityImages[name]) {
        ...
        entityImages[name].draw = function (entity) {
          if (this.loaded) {
            const name = entity.entity.name;
            let image; let sprWidth; let sprHeight; let offLeft; let offTop; let size; let offsetX; let
              offsetY;
            let flipX = false; let
              flipY = false;
            let rotation = 0;
            ...
            if (entityDrawRules[name] && (entityDrawRules[name].positionOffset || entityDrawRules[name].spritesheet)) {
              const rules = entityDrawRules[name];
              ...
              if (rules.spritesheet && Array.isArray(rules.spritesheet)) {
                const dir = Number(entity.entity.rot || 0);
                // 2 animated sprites, we will need to flip it for rotations.
                if (rules.spritesheet.length == 2) {
                  const spritesheetAnim = dir / 2 % 2;
                  const animationStage = Math.floor(Date.now() / 16) % rules.spritesheet[spritesheetAnim].spritesheet.length;
                  ...
                  if (rules.spritesheet[spriteSheetRotIndex].flip) {
                    const flip = rules.spritesheet[spriteSheetRotIndex].flip;
            ...
            const xPos = ((entity.x + offsetX) * remoteMapConfig.tileSize) - playerPosition.x;
            const yPos = ((entity.y + offsetY) * remoteMapConfig.tileSize) - playerPosition.y;

...
const cachedInstanceNames = {};
async function getInstanceName(instanceID) {
  const instance = cachedInstanceNames[instanceID];
  if (!instance) {
    const slaves = await getJSON('/api/slaves');
    try {
      for (id in slaves) {
...
function getImageFromName(name) {
  ...
  } return '/pictures/unknown-item.png';
}

...
describe('Master server endpoint testing', () => {
  describe('#GET /api/getFactorioLocale', () => {
    it('should get the basegame factorio locale', (done) => {
      request(app).get('/api/getFactorioLocale').end((err, res) => {
  ...
  parallel('#GET static website data', function () {
    ...
    it('sends some HTML when accessing /nodes', function (done) {
      this.timeout(6000);
      request(app).get('/nodes').end((err, res) => {
        assert.equal(res.statusCode, 200);
        validateHTML(res.text).then((result) => {
          assert(result.messages.length === 1, `there are HTML errors on the page, please fix: ${JSON.stringify(result.messages)}`);
          done();
    ...
    it('sends some HTML when accessing /settings', function (done) {
      this.timeout(6000);
      request(app).get('/settings').end((err, res) => {
        assert.equal(res.statusCode, 200);
        validateHTML(res.text).then((result) => {
          assert(result.messages.length === 1, `there are HTML errors on the page, please fix: ${JSON.stringify(result.messages)}`);
          done();
    ...
    it('sends some HTML when accessing /nodeDetails', function (done) {
      this.timeout(6000);
      request(app).get('/nodeDetails').end((err, res) => {
        assert.equal(res.statusCode, 200);
        validateHTML(res.text).then((result) => {
          assert(result.messages.length === 1, `there are HTML errors on the page, please fix: ${JSON.stringify(result.messages)}`);
          done();
    ...
    it('sends some HTML when accessing /remoteMap', function (done) {
      this.timeout(6000);
      request(app).get('/remoteMap').end((err, res) => {
        assert.equal(res.statusCode, 200);
        validateHTML(res.text).then((result) => {
  ...
  const persistentMaster = request(app);
  describe('#POST /api/place', () => {
    it('adds an itemStack to the masters inventory', (done) => {
  ...
  describe('#GET /api/inventory', () => {
    it('returns the masters current inventory', (done) => {
      persistentMaster.get('/api/inventory').end((err, res) => {
        assert.equal(res.statusCode, 200);
        const inventory = JSON.parse(res.text);
  ...
  parallel('#POST /api/remove', () => {
    it('returns an itemStack of how many items were removed', (done) => {
    ...
    it('returns an empty itemStack if you try to request addItem or removeItem', (done) => {
      let casesRan = 0;
      persistentMaster.post('/api/remove')
        .send({
          name  : 'addItem',
    ...
    it("returns an empty itemStack if you don't have any of the item you request", (done) => {
      persistentMaster.post('/api/remove')
        .send({
          name  : 'imaginaryItem',
          count : 999999,

    ...
    this.socket.on('gameChat', async (data) => {
      if (data.instanceID && data.instanceID != this.config.unique.toString()) {
        if (data.data.includes('[CHAT]')
          ...
          const words = data.data.trim().split(' ');
          words.shift();
          words.shift();
          words.shift();
          const message = words.join(' ').replace(/[";]/g, ' ').replace(/[']/g, '').replace('/shout ', '')
            .replace('!shout', '');

          const instance = await this.getInstanceName(data.instanceID);
      ...
      if (!instance) {
        needle.get(`${this.config.masterIP}:${this.config.masterPort}/api/slaves`, (err, response) => {
          if (err || response.statusCode != 200) {
            ...
            try {
              for (const index in response.body) this.instances[index] = response.body[index].instanceName;
            } catch (e) {
              console.log(e);
              return null;
            }

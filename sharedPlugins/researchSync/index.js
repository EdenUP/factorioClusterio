...
class ResearchSync {
  ...
  doSync() {
    needle.get(`${this.config.masterIP}:${this.config.masterPort}/api/slaves`, (err, resp, slaveData) => {
      ...
      Object.keys(slaveData).forEach((instanceKey) => {
        ...
        const researchList = slaveData[instanceKey].meta.research;
        if (researchList) {
      ...
      const difference = this.filterResearchDiff(this.research, needResearch);
      ...
      }, { headers : { 'x-access-token' : this.config.masterAuthToken } }, (err, resp) => {
        // success?
      });
    });
  }
  ...
  scriptOutput(data) {
    const kv              = data.split(':');
    const name            = kv[0];
    const researched      = (kv[1] !== 'true'
      ? 0
      : 1);
    const level           = parseInt(kv[2]);

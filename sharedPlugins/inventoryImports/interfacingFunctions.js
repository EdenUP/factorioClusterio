...
function handleInventory(json, config, callback) {
  ...
  if (json && typeof json !== 'string' && typeof json === 'object') {
    ...
    // {"players":{"1":{"inventory":{"stone":12,"iron-ore":120,"raw-wood":10,"iron-plate":8,"steel-plate":480},"requestSlots":{}}}}
    if (json.players) {
      const playerNames = Object.keys(json.players);
      for (let i = 0; i < playerNames.length; i++) {
        // haha, this turned into bit of a mess didn't it?
        const playerInv = json.players[playerNames[i]];
        // create a copy of player.requestSlots
        const toRequest = objectOps.deepclone(playerInv.requestSlots);
        const inventory = playerInv.inventory;
        let itemsToConfirm = Object.keys(toRequest).length;
        // create a place to store the object we have confirmed for importing
        const confirmedItems = {};
        const collectStacks = function (stack) {
          itemsToConfirm--;
          if (stack) {
        ...
        Object.keys(playerInv.requestSlots).forEach((key) => {
          ...
          if (toRequest[key] > 0) {
            ...
            needle.post(`${config.masterIP}:${config.masterPort}/api/remove`, request, { json : true, headers : { 'x-access-token' : config.masterAuthToken } }, (err, response, body) => {
              if (!err && response && response.body && (typeof response.body === 'object' || isJson(response.body))) {
                ...
                const stack = { name : response.body.name, count : Number(response.body.count) };
                collectStacks(stack);
              } else {
                collectStacks(false);
              }
    ...
    if (json.exports) {
      ...
      Object.keys(json.exports).forEach((playerName) => {
        // loop over simpleItemStacks to export
        const stackArray = json.exports[playerName];
        for (let i = 0; i < stackArray.length; i++) {
          if (stackArray[i].count > 0) {
            // console.log("Returning overflow: " + JSON.stringify(stackArray[i]));
            needle.post(`${config.masterIP}:${config.masterPort}/api/place`, stackArray[i], { headers : { 'x-access-token' : config.masterAuthToken } }, (err, resp, body) => {
              // console.log(body);
            });
          }
        }
...
function insertItemsFromObject(confirmedItems, playerName) {
  ...
  if (playerName && confirmedItems) {
    ...
    let itemTable = '{';
    Object.keys(confirmedItems).forEach((name) => {
      const count = confirmedItems[name];
      if (count > 0) {
        itemTable += `["${name}"]=${Math.floor(count)},`;
      }
    });
    itemTable += '}';
    const outputFile = pluginConfig.scriptOutputFileSubscription;
    // a="'+players[i]+'"
    // b='+itemTable+'
